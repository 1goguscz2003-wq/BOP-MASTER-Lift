(()=>{'use strict';
  ...
  function adminTabs(){return`<div class="tabs no-print">${[['overview','Обзор'],['accounts','Аккаунты'],['qr','Кюар-коды'],['history','История']].map(([id,label])=>`<button class="tab ${adminTab===id?'active':''}" data-tab="${id}">${label}</button>`).join('')}</div>`}
  async function renderAdmin(silent=false){
    ...
    try{
      ...
      if(adminTab==='qr'){const qrs=await api('admin_qrs',{accessKey:session.key});await loadQr();const base=new URL('./w.html',location.href).href.split('?')[0];body=`<section class="card qr-print"><div class="screen-head no-print"><h2>Кюар-коды этажей</h2><div class="actions"><button id="print-qrs" class="button secondary">Печать кюаров</button><button id="rotate-qrs" class="button danger">Пересоздать кюары</button></div></div><div class="qr-grid">${qrs.map(q=>{const url=base+'?f='+q.floor+'&t='+encodeURIComponent(q.token);return`<article class="qr-card"><h3>Этаж ${q.floor}</h3>${qrSvg(url)}</article>`}).join('')}</div></section>`}
  ...
  function bindAdmin(dashboard){
    ...
    const rotateQr=document.getElementById('rotate-qrs');if(rotateQr)rotateQr.onclick=async()=>{if(!confirm('Пересоздать все 7 кюаров? Старые сразу перестанут работать.'))return;rotateQr.disabled=true;rotateQr.textContent='Пересоздаём…';try{await api('admin_rotate_qrs',{accessKey:session.key});toast('Все 7 кюаров пересозданы');await renderAdmin()}catch(e){if(e.status!==401)toast('Не получилось пересоздать кюары')}finally{rotateQr.disabled=false;rotateQr.textContent='Пересоздать кюары'}};
  }
  ...
  function qrSvg(url){const code=window.qrcode(0,'M');code.addData(url,'Byte');code.make();return code.createSvgTag({cellSize:8,margin:32,scalable:true,alt:'Кюар-код вызова лифта'})}
  ...
  if('serviceWorker'in navigator)navigator.serviceWorker.register('./sw.js?v=20260903.5',{updateViaCache:'none'}).catch(()=>{});
